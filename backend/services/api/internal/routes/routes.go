package routes

import (
	"ava/api/internal/controller"
	"ava/api/internal/middleware"

	"github.com/gofiber/fiber/v2"
)

func AddRoutes(app *fiber.App, ctrl *controller.Controller, mw *middleware.Middleware) {
	app.Use(middleware.GlobalRateLimit())

	api := app.Group("/api/v1")

	healthRoutes(api.Group("/health"), ctrl.Health)

	authRoutes(api.Group("/auth"), ctrl.Auth, mw)
	apiTokenRoutes(api.Group("/tokens"), ctrl.APIToken, mw)
	tenantRoutes(api.Group("/tenants"), ctrl.Tenant, mw)
	flowRoutes(api.Group("/flows"), ctrl.Flow, mw)
	hubRoutes(api.Group("/hubs"), ctrl.Hub, ctrl.Device, mw)
	roomRoutes(api.Group("/rooms"), ctrl.Room, mw)
	sceneRoutes(api.Group("/rooms/:roomID/scenes"), ctrl.Scene, mw)
	deviceRoutes(api.Group("/devices"), ctrl.Device, mw)
	socketRoutes(api.Group("/socket"), ctrl.Socket, mw)
}
