...
func TestEveryIssuedTokenSplitsBackIntoItsOwnHalves(t *testing.T) {
	for range 500 {
		...
		lookup, secret, ok := model.SplitToken(plaintext)
		if !ok {
			t.Fatalf("could not split %q", plaintext)
		}

		if lookup != token.Lookup {
			t.Fatalf("split gave lookup %q, token stores %q (from %q)", lookup, token.Lookup, plaintext)
		}

		if !token.MatchesSecret(secret) {
			t.Fatalf("split gave a secret that does not match, from %q", plaintext)
		}
	}
}
